...
// Animation Imgs
const imgCardFront = document.querySelector(".card-front");
const imgCardBack = document.querySelector(".card-back");
const imgCardContainer = document.querySelector(".credit-card-container");
const attriContainer = document.querySelector(".attribution");
...
const startAnimation = (entries, observe) => {
  entries.forEach((entry) => {
    if (
      ...
      !entry.target.classList.contains("form-state")
    ) {
      entry.target.classList.add("show");
      setInterval(() => {
        entry.target.classList.remove("show");
      }, 3000);
    } else if (entry.target.classList.contains("form-state")) {
      entry.target.classList.add("visible-form");
      setInterval(() => {
        entry.target.classList.remove("visible-form");
      }, 2000);
    }
  });
};
...
observer.observe(imgCardFront);
observer.observe(imgCardBack);
observer.observe(imgCardContainer);
observer.observe(formContainer);
observer.observe(attriContainer);
